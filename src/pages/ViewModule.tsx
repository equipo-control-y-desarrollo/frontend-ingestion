...
export default function ViewModule() {
  ...
  const data: any = JSON.parse(localStorage.getItem("module") || "");
  ...
  const downloadExcel = async (): Promise<any> => {
    ...
    try {
      ...
      const message = JSON.parse(enc.decode(err.response.data)).message;
            ...
            onClick={() =>
              navigate(`../addRow/${module}`, {
                state: {
                  id: -1,
                  isEdit: false,
                  previous_data: rows.length === 0 ? { saldo: 0 } : rows[0],
                },
              })
            }
          >
